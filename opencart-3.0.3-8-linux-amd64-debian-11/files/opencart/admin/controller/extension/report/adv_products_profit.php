...
class ControllerExtensionReportAdvProductsProfit extends Controller { 
	private $error = array();
	
	public function index() {
		$this->load->language('extension/report/adv_products_profit');
		
		$query1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "extension WHERE code = 'adv_profit_module'");
		if (empty($query1->num_rows)) {	
			$this->session->data['error_ppp'] = $this->language->get('error_installed1');
		...
		if (empty($query2->num_rows)) {	
			$this->session->data['error_ppp'] = $this->language->get('error_installed2');
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));		
		}
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_filters')) {
			$advppp_settings_filters = $this->config->get('advppp' . $this->user->getId() . '_settings_filters');
		} else {
			$advppp_settings_filters = array();
		}
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_all_columns')) {
			$advppp_settings_all_columns = $this->config->get('advppp' . $this->user->getId() . '_settings_all_columns');
		} else {
		...
		if (in_array('scw_upc', $advppp_settings_scw_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_upc'),
			'value' => 'upc',
		);
		}
		if (in_array('scw_ean', $advppp_settings_scw_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_ean'),
			'value' => 'ean',
		);
		}
		if (in_array('scw_jan', $advppp_settings_scw_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_jan'),
			'value' => 'jan',
		);
		}
		if (in_array('scw_isbn', $advppp_settings_scw_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_isbn'),
			'value' => 'isbn',
		);
		}
		if (in_array('scw_mpn', $advppp_settings_scw_columns)) {		
		$data['sort'][] = array(
		...
		if (in_array('pnp_upc', $advppp_settings_pnp_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_upc'),
			'value' => 'upc',
		);
		}
		if (in_array('pnp_ean', $advppp_settings_pnp_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_ean'),
			'value' => 'ean',
		);
		}
		if (in_array('pnp_jan', $advppp_settings_pnp_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_jan'),
			'value' => 'jan',
		);
		}
		if (in_array('pnp_isbn', $advppp_settings_pnp_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_isbn'),
			'value' => 'isbn',
		);
		}
		if (in_array('pnp_mpn', $advppp_settings_pnp_columns)) {		
		$data['sort'][] = array(
			'text'  => $this->language->get('column_mpn'),
			'value' => 'mpn',
		);
		...
		if (!$advppp_settings_cm_columns or (in_array('cm_orders', $advppp_settings_cm_columns))) {			
		$data['sort'][] = array(
		...
		if (!$advppp_settings_mv_columns or (in_array('mv_orders', $advppp_settings_mv_columns))) {			
		$data['sort'][] = array(
			'text'  => $this->language->get('column_orders'),
			'value' => 'orders',
		);
		...
		$data['order_statuses'] = $this->model_extension_report_adv_products_profit->getOrderStatuses(); 	
		...
		$data['stores'] = $this->model_extension_report_adv_products_profit->getOrderStores();
		...
		$data['currencies'] = $this->model_extension_report_adv_products_profit->getOrderCurrencies();
		...
		$data['taxes'] = $this->model_extension_report_adv_products_profit->getOrderTaxes();	
		...
		$data['tax_classes'] = $this->model_extension_report_adv_products_profit->getOrderTaxClasses();
		...
		$data['geo_zones'] = $this->model_extension_report_adv_products_profit->getOrderGeoZones();
		...
		$data['customer_groups'] = $this->model_extension_report_adv_products_profit->getOrderCustomerGroups();	
		...
		$data['payment_methods'] = $this->model_extension_report_adv_products_profit->getOrderPaymentMethods();	
		...
		$data['shipping_methods'] = $this->model_extension_report_adv_products_profit->getOrderShippingMethods();	
		...
		$data['categories'] = $this->model_extension_report_adv_products_profit->getProductsCategories(0);
		...
		$data['manufacturers'] = $this->model_extension_report_adv_products_profit->getProductsManufacturers(); 
		...
		$data['suppliers'] = $this->model_extension_report_adv_products_profit->getProductsSuppliers(); 
		...
		if ($advppp_settings_filters and (in_array('option', $advppp_settings_filters))) {		
		$data['order_options'] = $this->model_extension_report_adv_products_profit->getProductOptions();
		...
		if ($advppp_settings_filters and (in_array('attribute', $advppp_settings_filters))) {		
		$data['attributes'] = $this->model_extension_report_adv_products_profit->getProductAttributes();
		...
		$data['product_statuses'] = $this->model_extension_report_adv_products_profit->getProductStatuses();	
		...
		$data['locations'] = $this->model_extension_report_adv_products_profit->getProductLocations();
		...
		if ($advppp_settings_filters and (in_array('coupon_name', $advppp_settings_filters))) {		
		$data['coupon_names'] = $this->model_extension_report_adv_products_profit->getOrderCouponNames();
		...
		if ($advppp_settings_filters and (in_array('campaign_name', $advppp_settings_filters))) {		
		$data['campaign_names'] = $this->model_extension_report_adv_products_profit->getOrderCampaignNames();
		...
		if (isset($_GET['cron'])) {
			$cron_settings = $this->config->get('advppp' . $this->user->getId() . 'cron_setting') !== null ? unserialize($this->config->get('advppp' . $this->user->getId() . 'cron_setting')) : '';
		...
		if (isset($this->request->get['filter_campaign_code'])) {
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/report/adv_products_profit', 'user_token=' . $this->session->data['user_token'], true)
   		);
		...
		$selected_load_save_reports = $this->config->get('advppp' . $this->user->getId() . 'sr_load_save_report') !== null ? unserialize($this->config->get('advppp' . $this->user->getId() . 'sr_load_save_report')) : '';
		...
		$selected_cron_settings = $this->config->get('advppp' . $this->user->getId() . 'cron_setting') !== null ? unserialize($this->config->get('advppp' . $this->user->getId() . 'cron_setting')) : '';
		...
		$results = $this->model_extension_report_adv_products_profit->getProductsProfit($filter_data);
		$totals = $filter_limit != '999999' ? $this->model_extension_report_adv_products_profit->getProductsProfitTotal($filter_data) : FALSE;
		...
		if ($filter_details != 'all_details_products' && $filter_details != 'all_details_orders') {
		  ...
		  if ($filter_details == 'basic_details' && ($filter_report == 'manufacturers' or $filter_report == 'categories' or $filter_report == 'suppliers' or $filter_report == 'products_options')) {
			$details = array(
				...
				'product_manu'  					=> ($filter_report == 'categories' || $filter_report == 'suppliers' || $filter_report == 'products_options') ? $result['product_manu'] : '',
				'product_category'  				=> ($filter_report == 'manufacturers' || $filter_report == 'suppliers' || $filter_report == 'products_options') ? $result['product_category'] : '',
				'product_supplier'  				=> ($filter_report == 'manufacturers' || $filter_report == 'categories' || $filter_report == 'products_options') ? $result['product_supplier'] : '',
			...
			$manufacturer = $this->model_extension_report_adv_products_profit->getProductManufacturers($result['manufacturer_id']);
			$supplier = $this->model_extension_report_adv_products_profit->getProductSuppliers($result['supplier_id']);
			...
			$data['discount_total'] = $this->currency->format($discount_total, $this->config->get('config_currency'));
			...
			$data['total_sales_total'] = $this->currency->format($total_sales_total, $this->config->get('config_currency'));
			$data['total_costs_total'] = $this->currency->format($total_costs_total, $this->config->get('config_currency'));
			$data['total_profit_margin_total'] = $total_profit_margin_total;
			$data['total_profit_markup_total'] = $total_profit_markup_total;
					...
					foreach (json_decode($result['option']) as $product_option_id => $value) {
						...
						if ($option_query->num_rows) {
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
								$option_value_query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, poc.cost, poc.cost_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) LEFT JOIN " . DB_PREFIX . "product_option_cost poc ON (pov.product_option_value_id = poc.product_option_value_id) WHERE pov.product_option_value_id = '" . (int)$value . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
			...
			$option_cost = 0;
			$option_data = array();
			$options = $this->model_extension_report_adv_products_profit->getOrderOptions($result['order_product_id']);

			foreach ($options as $option) {
				if ($option['type'] != 'file' && $option['type'] != 'image' && $option['type'] != 'date' && $option['type'] != 'datetime' && $option['type'] != 'time') {
					$option_value_query = $this->db->query("SELECT pov.quantity, pov.price, pov.price_prefix, poc.cost, poc.cost_prefix FROM " . DB_PREFIX . "product_option_value pov, " . DB_PREFIX . "product_option_cost poc WHERE pov.product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND pov.product_option_value_id = poc.product_option_value_id AND pov.product_option_id = '" . (int)$option['product_option_id'] . "'");
					...
					if ($option_value_query->num_rows) {
						...
						$option_data[] = array(
							...
							'price_prefix'      	=> $option_value_query->row['price_prefix'],
							'cost'             		=> $option_value_query->row['cost'],
							'fcost'             	=> $this->currency->format($option_value_query->row['cost'], $this->config->get('config_currency')),
							'cost_prefix'     		=> $option_value_query->row['cost_prefix']										
						);
			...
			$products[] = array(
				...
				'gcategories'  						=> $result['categories'] !== null ? html_entity_decode($result['categories']) : '',
				'gmanufacturer'    					=> $result['manufacturer'] !== null ? html_entity_decode($result['manufacturer']) : '',
				'gsupplier'    						=> $result['supplier'] !== null ? html_entity_decode($result['supplier']) : '',
				'gsold'    							=> $result['sold_quantity'],				
				'gsales'      						=> $total_sales !== null ? round($total_sales, 2) : 0,
				'gcosts'      						=> $total_costs !== null ? round($total_costs, 2) : 0,
			...
			// Taxes
			$tax_data = array();
			$taxes = $this->model_extension_report_adv_products_profit->getOrderTaxesDivided($result['order_id']);
			...
			// Custom Fields
			if (in_array('all_custom_fields', $advppp_settings_all_columns)) {		
			...
			$data['custom_fields'] = $this->model_extension_report_adv_products_profit->getCustomFieldsNames($filter_data);			
			...
			$products[] = array(
				...
				'product_refund'  				=> $this->currency->format($data['advppp_formula_6'] ? '-' . $result['product_refund'] : $result['product_refund'], $result['currency_code'], $result['currency_value']),
				...
				'order_refund'      			=> $this->currency->format($data['advppp_formula_6'] ? '-' . $result['order_refund'] : $result['order_refund'], $result['currency_code'], $result['currency_value']),
				...
				'order_product_costs'   		=> $this->currency->format('-' . ($result['order_product_costs']-($data['advppp_formula_7'] ? $result['order_product_costs_returns'] : 0)), $result['currency_code'], $result['currency_value']),
				'order_product_costs_raw'   	=> $result['order_product_costs_raw']-($data['advppp_formula_7'] ? $result['order_product_costs_returns_raw'] : 0),
		...
		if (!isset($_GET['cron'])) {
			...
			file_put_contents('ppp_json_array.txt', json_encode($data['products']));
		}
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_filters')) {
			...
			$data['advppp_settings_filters'] = array('store','currency','tax','tax_class','geo_zone','customer_group','customer_name','customer_email','customer_telephone','payment_company','payment_address','payment_city','payment_zone','payment_postcode','payment_country','payment_method','shipping_company','shipping_address','shipping_city','shipping_zone','shipping_postcode','shipping_country','shipping_method','category','manufacturer','supplier','product','sku','model','product_status');
		}
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_cm_columns')) {
			...
			$data['advppp_settings_cm_columns'] = array('cm_category','cm_manufacturer','cm_supplier','cm_orders','cm_sold_quantity','cm_sold_percent','cm_total_excl_vat','cm_total_tax','cm_total_incl_vat','cm_app','cm_refunds','cm_total_sales','cm_total_costs','cm_total_profit','cm_total_profit_margin','cm_total_profit_markup');
		}
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_mv_columns')) {
			...
			$data['advppp_settings_mv_columns'] = array('mv_id','mv_image','mv_name','mv_model','mv_category','mv_manufacturer','mv_status','mv_stock_quantity','mv_orders','mv_sold_quantity','mv_sold_percent','mv_total_excl_vat','mv_total_tax','mv_total_incl_vat','mv_app','mv_refunds','mv_total_sales','mv_total_costs','mv_total_profit','mv_total_profit_margin','mv_total_profit_markup');
		}	
		...
		if ($this->config->get('advppp' . $this->user->getId() . '_settings_pl_columns')) {
			...
			$data['advppp_settings_pl_columns'] = array('pl_prod_order_id','pl_prod_date_added','pl_prod_id','pl_prod_sku','pl_prod_model','pl_prod_name','pl_prod_option','pl_prod_currency','pl_prod_price','pl_prod_quantity','pl_prod_total_excl_vat','pl_prod_tax','pl_prod_total_incl_vat','pl_prod_sales','pl_prod_cost','pl_prod_profit','pl_prod_profit_margin','pl_prod_profit_markup');
		}
		...
		$data['initialise'] = '';
		 if (!$query->rows) {
			...
			$settings_data = array(
				'advppp' . $this->user->getId() . '_settings_filters' 		=> array('store','currency','tax','tax_class','geo_zone','customer_group','customer_name','customer_email','customer_telephone','payment_company','payment_address','payment_city','payment_zone','payment_postcode','payment_country','payment_method','shipping_company','shipping_address','shipping_city','shipping_zone','shipping_postcode','shipping_country','shipping_method','category','manufacturer','supplier','product','sku','model','product_status'),
				...
				'advppp' . $this->user->getId() . '_settings_cm_columns' 	=> array('cm_category','cm_manufacturer','cm_supplier','cm_orders','cm_sold_quantity','cm_sold_percent','cm_total_excl_vat','cm_total_tax','cm_total_incl_vat','cm_app','cm_refunds','cm_total_sales','cm_total_costs','cm_total_profit','cm_total_profit_margin','cm_total_profit_markup'),								
				'advppp' . $this->user->getId() . '_settings_mv_columns' 	=> array('mv_id','mv_image','mv_name','mv_model','mv_category','mv_manufacturer','mv_status','mv_stock_quantity','mv_orders','mv_sold_quantity','mv_sold_percent','mv_total_excl_vat','mv_total_tax','mv_total_incl_vat','mv_app','mv_refunds','mv_total_sales','mv_total_costs','mv_total_profit','mv_total_profit_margin','mv_total_profit_markup'),	
				'advppp' . $this->user->getId() . '_settings_ol_columns' 	=> array_keys($data['ol_columns']),
				'advppp' . $this->user->getId() . '_settings_pl_columns' 	=> array('pl_prod_order_id','pl_prod_date_added','pl_prod_id','pl_prod_sku','pl_prod_model','pl_prod_name','pl_prod_option','pl_prod_currency','pl_prod_price','pl_prod_quantity','pl_prod_total_excl_vat','pl_prod_tax','pl_prod_total_incl_vat','pl_prod_sales','pl_prod_cost','pl_prod_profit','pl_prod_profit_margin','pl_prod_profit_markup'),
		...
		$pagination->url = $this->url->link('extension/report/adv_products_profit', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$data['report_link'] = $this->url->link('extension/report/adv_products_profit', 'user_token=' . $this->session->data['user_token'], true);
		$data['save_report_link'] = 'index.php?route=extension/report/adv_products_profit'. $url;
		...
		$data['url'] = $this->url->link('extension/report/adv_products_profit', 'user_token=' . $this->session->data['user_token'] . $url . '&page='. $page, true);
		...
		if (!isset($_GET['cron'])) {
			$this->response->setOutput($this->load->view('extension/report/adv_products_profit', $data));
		} else {	
			$export_data = array(
			...
			$cron_settings = $this->config->get('advppp' . $user_id . 'cron_setting') !== null ? unserialize($this->config->get('advppp' . $user_id . 'cron_setting')) : '';
			foreach ($cron_settings as $cron_setting) {
				if (isset($_GET['cron_id']) && ($_GET['cron_id'] == $cron_setting['cron_id'])) {
			...
			if ($report_type == 'export_no_details' && $export_type == 'export_xls') {
				...
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xls.inc.php');
				exit();
				...
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx_php8.inc.php');
				}		
				exit();	
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_csv.inc.php');
				exit();
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_pdf') {
				require_once(DIR_SYSTEM . 'library/dompdf/autoload.inc.php');
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_pdf.inc.php');
				exit();
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_html') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_html.inc.php');
				exit();	
				...
				require_once('Spreadsheet/Excel/Writer.php');
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xls_basic_details.inc.php');
				exit();
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_xlsx') {
				if (PHP_VERSION_ID < 80000) {			
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel.php');
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel/IOFactory.php');			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx_basic_details.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx_php8_basic_details.inc.php');
				}
				exit();	
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_csv_basic_details.inc.php');
				exit();				
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_pdf') {
				require_once(DIR_SYSTEM . 'library/dompdf/autoload.inc.php');
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_pdf_basic_details.inc.php');	
				exit();
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_html') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_html_basic_details.inc.php');
				exit();	
				...
				require_once('Spreadsheet/Excel/Writer.php');
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xls_all_details.inc.php');
				exit();
			} elseif (($report_type == 'export_all_details_products' or $report_type == 'export_all_details_orders') && $export_type == 'export_xlsx') {
				if (PHP_VERSION_ID < 80000) {			
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel.php');
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel/IOFactory.php');			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx_all_details.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_xlsx_php8_all_details.inc.php');
				}
				exit();	
			} elseif (($report_type == 'export_all_details_products' or $report_type == 'export_all_details_orders') && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/ppp_export_csv_all_details.inc.php');
				exit();			
			} else {
				exit();
			}		
	...
	public function customer_autocomplete() {
		...
		$results = $this->model_extension_report_adv_products_profit->getCustomerAutocomplete($filter_data);
	...
	public function product_autocomplete() {
		...
		$results = $this->model_extension_report_adv_products_profit->getProductAutocomplete($filter_data);
	...
	public function coupon_autocomplete() {
		...
		$results = $this->model_extension_report_adv_products_profit->getCouponAutocomplete($filter_data);
	...
	public function voucher_autocomplete() {
		...
		$results = $this->model_extension_report_adv_products_profit->getVoucherAutocomplete($filter_data);
	...
	public function campaign_autocomplete() {
		...
		$results = $this->model_extension_report_adv_products_profit->getCampaignAutocomplete($filter_data);
	...
	public function settings($filter_data = array()) {
		...
		if (!$json) {
			if (!$this->user->hasPermission('modify', 'extension/report/adv_products_profit')) {
	...
	public function cron($filter_data = array()) {
		...
		$crons = unserialize($this->config->get('advppp' . $this->user->getId() . 'cron_setting'));
	...
	public function export($filter_data = array()) {
		$this->load->language('extension/report/adv_products_profit');
		$product_data = json_decode(file_get_contents('ppp_json_array.txt'), true);
