<?php
class ControllerExtensionModuleAdvReportsProductsProfit extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->config->get('module_adv_reports_products_profit_status')) {
			return;
		}
	...
	public function index() {  		
		$this->load->language('extension/module/adv_reports_products_profit');
		...
		$data['button_cancel'] = $this->language->get('button_cancel');	
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/adv_reports_products_profit', 'user_token=' . $this->session->data['user_token'], true)
   		);
		...
		$this->response->setOutput($this->load->view('extension/module/adv_reports_products_profit', $data));
	}
	
	public function install(){
		$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET store_id = '0', `code` = 'module_adv_reports_products_profit', `key` = 'module_adv_reports_products_profit_status', `value` = '1'");
		$this->load->model('user/user_group');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/report/adv_products_profit');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/report/adv_products_profit');			
		// add event handlers
		$this->load->model('setting/event');
		$code = 'module_adv_reports_products_profit';
		$app = 'admin/';
		$trigger = 'view/*/before';
		$route = 'extension/module/adv_reports_products_profit/override';
		$this->model_setting_event->addEvent( $code, $app.$trigger, $route, 1, 0 );
		$trigger = 'view/design/layout_form/before';
		$route = 'extension/module/adv_reports_products_profit/eventViewDesignLayoutFormBefore';
		$this->model_setting_event->addEvent( $code, $app.$trigger, $route, 1, 0 );		
	}
	
	public function uninstall() {
		$this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE store_id = '0' AND code = 'advppp" . $this->user->getId() . "'");
	...
	public function eventViewDesignLayoutFormBefore( &$route, &$data, &$template ) {
		foreach ($data['extensions'] as $key=>$extension) {
			if ($extension['code'] == 'module_adv_reports_products_profit') {
