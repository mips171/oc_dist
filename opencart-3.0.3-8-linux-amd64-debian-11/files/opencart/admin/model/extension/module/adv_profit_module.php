<?php
class ModelExtensionModuleAdvProfitModule extends Model {
	public function getProductCostPriceQuantity($start, $limit, $categoryfilter, $manufacturerfilter, $supplierfilter, $statusfilter, $roundingfilter, $sort) {
		...
		$sql = "SELECT p.product_id, p.quantity, pd.name, p.model, p.sku, p.price, p.subtract, pc.cost, pc.cost_amount, pc.cost_percentage, pc.cost_additional, pc.costing_method, pc.supplier_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_cost pc ON (pc.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (pd.product_id = p.product_id)";
		...
		if (!empty($supplierfilter)) {
			...
			foreach ($supplierfilter as $suppliers_filter) {
				$implode[] = "pc.supplier_id = '" . (int)$suppliers_filter . "'";
			}
		...
		foreach ($query->rows as $result) {
			...
			foreach ($product_option_query->rows as $product_option) {
				if ($product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'select' || $product_option['type'] == 'image' || $product_option['type'] == 'colour' || $product_option['type'] == 'size' || $product_option['type'] == 'multiple') {
					...
					$product_option_value_query = $this->db->query("SELECT pov.product_id, pov.product_option_value_id, ovd.name AS option_value, od.name AS option_name, poc.sku, pov.quantity, poc.costing_method, poc.cost_amount, poc.cost, pov.price, pov.subtract FROM " . DB_PREFIX . "product_option_value pov, " . DB_PREFIX . "product_option_cost poc, " . DB_PREFIX . "option_value ov, " . DB_PREFIX . "option_value_description ovd, " . DB_PREFIX . "option_description od WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND pov.product_option_value_id = poc.product_option_value_id AND pov.option_value_id = ov.option_value_id AND ov.option_value_id = ovd.option_value_id AND ovd.option_id = od.option_id AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
		...
		foreach ($products as $product) {
		   ...
		   $costing_method = strtoupper($product['costing_method']) == 'AVCO' ? 1 : 0;
			...
			if ($product['option_id'] != 0) { //update item at option level
				$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = " . $product['quantity'] . ", price = " . (float)$product['price'] . ", subtract = " . $subtract . " WHERE product_option_value_id = " . $product['option_id'] . ";");
				$this->db->query("UPDATE " . DB_PREFIX . "product_option_cost SET cost = " . (float)$product['cost'] . ", cost_amount = " . (float)$product['restock_cost'] . ", costing_method = " . $costing_method . " WHERE product_option_value_id = " . $product['option_id'] . ";");
				...
				$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = " . (int)$product['quantity'] . ", price = " . (float)$product['price'] . ", subtract = " . $subtract . " WHERE product_id = " . $product['product_id'] . ";");
				$this->db->query("UPDATE " . DB_PREFIX . "product_cost SET supplier_id = " . (int)$product['supplier_id'] . ", cost = " . (float)$product['cost'] . ", cost_amount = " . (float)$product['restock_cost'] . ", cost_percentage = '0.00', cost_additional = '0.0000', costing_method = " . $costing_method . " WHERE product_id = " . $product['product_id'] . ";");
	...
	public function createXLS($categoryfilter, $manufacturerfilter, $supplierfilter, $statusfilter, $roundingfilter) {
		...
		$config = $this->config;
		$log = $this->log;
		set_error_handler('error_handler_for_stock_export',E_ALL);
		register_shutdown_function('fatal_error_shutdown_handler_for_stock_export');
    	...
    	$results = $this->model_extension_module_adv_profit_module->getProductCostPriceQuantity(1, 100000, $categoryfilter, $manufacturerfilter, $supplierfilter, $statusfilter, $roundingfilter, '');
			...
			$excelRow = $i+1;			
				...
				if ($roundingfilter == 'RD10DW') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDDOWN((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0)-0.10,0)', $priceNewFormat);					
				} elseif ($roundingfilter == 'RD5DW') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDDOWN((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0)-0.05,0)', $priceNewFormat);	
				} elseif ($roundingfilter == 'RD1DW') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDDOWN((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0)-0.01,0)', $priceNewFormat);	
				} elseif ($roundingfilter == 'RD00DW') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDDOWN((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0),0)', $priceNewFormat);					
				} elseif ($roundingfilter == 'RD0DW') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDDOWN((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),1),0)', $priceNewFormat);				
				} elseif ($roundingfilter == 'RD') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,(O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0)', $priceNewFormat);
				} elseif ($roundingfilter == 'RD0UP') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDUP((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),1),0)', $priceNewFormat);
				} elseif ($roundingfilter == 'RD10UP') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDUP((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .')+0.01,0)-0.10,0)', $priceNewFormat);
				} elseif ($roundingfilter == 'RD5UP') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDUP((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .')+0.01,0)-0.05,0)', $priceNewFormat);
				} elseif ($roundingfilter == 'RD1UP') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDUP((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .')+0.01,0)-0.01,0)', $priceNewFormat);
				} elseif ($roundingfilter == 'RD00UP') {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,ROUNDUP((O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0),0)', $priceNewFormat);
				} else {
					$worksheet->writeFormula($i, $j++, '=IF(((Q' . $excelRow . ')+(R' . $excelRow .')) > 0,(O' . $excelRow . '*Q' . $excelRow . ')+(P' . $excelRow . '*R' . $excelRow .')+(S' . $excelRow . '-P' . $excelRow .'),0)', $priceNewFormat);
				}
