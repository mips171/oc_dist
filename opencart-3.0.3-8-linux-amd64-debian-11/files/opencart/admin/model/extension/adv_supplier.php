<?php
class ModelExtensionAdvSupplier extends Model {
	...
	public function getSuppliers($data = array()) {
		$sql = "SELECT *, (SELECT COUNT(pc.supplier_id) FROM `" . DB_PREFIX . "product_cost` pc WHERE s.supplier_id = pc.supplier_id) AS products FROM " . DB_PREFIX . "supplier s";
	...
	public function getTotalProductsBySupplierId($supplier_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_cost WHERE supplier_id = '" . (int)$supplier_id . "'");
