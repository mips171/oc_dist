<?php
class ModelExtensionReportAdvSalesProfit extends Model {
	public function getSalesProfit($data = array()) {
		...
		if (!empty($data['filter_supplier'])) {
			$supl .= " AND (SELECT DISTINCT opc.order_id FROM `" . DB_PREFIX . "order_product_cost` opc WHERE o.order_id = opc.order_id AND (";
			$implode = array();
			foreach ($data['filter_supplier'] as $supplier) {
				$implode[] = "opc.supplier_id = '" . (int)$supplier . "'";
			}
			if ($implode) {
		...
		$opt = '';
		if ($this->config->get('advsop' . $this->user->getId() . '_settings_filters') and (in_array('option', $this->config->get('advsop' . $this->user->getId() . '_settings_filters')))) {		
		...
		$atr = '';
		if ($this->config->get('advsop' . $this->user->getId() . '_settings_filters') and (in_array('attribute', $this->config->get('advsop' . $this->user->getId() . '_settings_filters')))) {		
		...
		$cpn = '';
		if ($this->config->get('advsop' . $this->user->getId() . '_settings_filters') and (in_array('coupon_name', $this->config->get('advsop' . $this->user->getId() . '_settings_filters')))) {		
		...
		$cgn = '';
		if ($this->config->get('advsop' . $this->user->getId() . '_settings_filters') and (in_array('campaign_name', $this->config->get('advsop' . $this->user->getId() . '_settings_filters')))) {		
		...
		if (!$this->config->get('advsop' . $this->user->getId() . '_settings_mv_columns') or (in_array('mv_customers', $this->config->get('advsop' . $this->user->getId() . '_settings_mv_columns')))) {		
			$sql .= " ((SELECT COUNT(DISTINCT o.customer_id) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id AND o.customer_id != 0 GROUP BY o.customer_id) UNION ALL (SELECT COUNT(DISTINCT CONCAT(o.lastname,o.firstname)) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id AND o.customer_id = 0 GROUP BY CONCAT(o.lastname,o.firstname))) AS customers, ";
		} else {
			$sql .= " '' AS customers, ";	
		}
		$sql .= " SUM((SELECT SUM(op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id)) AS products, 
		...
		SUM((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND o.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id)) AS refunds,  
		SUM((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id)) AS prod_costs_returns, 
		SUM((SELECT SUM((opc.base_price-op.price)*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND op.order_product_id = opc.order_product_id GROUP BY op.order_id)) AS discount, 
		SUM(o.total) AS total, 
		SUM((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND op.order_product_id = opc.order_product_id GROUP BY op.order_id)) AS prod_costs, 
		SUM(o.commission) AS commission, 
		SUM((SELECT SUM(oc.payment_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id GROUP BY oc.order_id)) AS payment_cost, 
		SUM((SELECT SUM(oc.shipping_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id GROUP BY oc.order_id)) AS shipping_cost, 
		SUM((SELECT SUM(oc.extra_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id GROUP BY oc.order_id)) AS extra_cost, 
		SUM((SELECT SUM(rc.cost) FROM `" . DB_PREFIX . "return_cost` rc WHERE rc.order_id = o.order_id GROUP BY rc.order_id)) AS return_cost ";
		...
		if (isset($data['filter_details']) && $data['filter_details'] == 'basic_details') {
			...
			GROUP_CONCAT('<a href=\"index.php?route=sale/order/info&user_token=$user_token&order_id=',o.order_id,'\">',o.order_id,'</a>' ORDER BY o.order_id DESC SEPARATOR '<br>') AS order_ord_id_link, ";
			if ($this->config->get('advsop' . $this->user->getId() . '_date_format') == 'DDMMYYYY') {
			...
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND o.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}					
			$sql .= "), 2) ORDER BY o.order_id DESC SEPARATOR '<br>') AS order_sales, 
			GROUP_CONCAT(ROUND(o.currency_value*(IFNULL((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id),0)+o.commission";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
				$sql .= "+(SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {		
				$sql .= "+(SELECT oc.payment_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_4')) {		
				$sql .= "+(SELECT oc.extra_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_5')) {		
				$sql .= "+IFNULL((SELECT SUM(rc.cost) FROM `" . DB_PREFIX . "return_cost` rc WHERE rc.order_id = o.order_id GROUP BY rc.order_id),0)";
			}	
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {		
				$sql .= "-IFNULL((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}
			...
			$sql .= ")";					
			$sql .= "-(IFNULL((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id),0)+o.commission";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
				$sql .= "+(SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {	
			...
			$sql .= ")), 2),0) ORDER BY o.order_id DESC SEPARATOR '%<br>') AS order_profit_prc, 
			...
			GROUP_CONCAT((SELECT GROUP_CONCAT('<a href=\"index.php?route=sale/order/info&user_token=$user_token&order_id=',op.order_id,'\">',op.order_id,'</a>' SEPARATOR '<br>') FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '<br>') AS product_ord_id_link, ";
			if ($this->config->get('advsop' . $this->user->getId() . '_date_format') == 'DDMMYYYY') {
			...
			GROUP_CONCAT((SELECT GROUP_CONCAT(IFNULL((SELECT s.name FROM `" . DB_PREFIX . "supplier` s WHERE opc.supplier_id = s.supplier_id),'&nbsp;') SEPARATOR '&nbsp;<br>') FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id AND op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '<br>') AS product_supplier, 
			...
			GROUP_CONCAT((SELECT GROUP_CONCAT(ROUND(o.currency_value*(op.total";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			$sql .= "), 2) SEPARATOR '<br>') FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '<br>') AS product_sales, 
			GROUP_CONCAT((SELECT GROUP_CONCAT(ROUND(o.currency_value*((opc.cost*op.quantity)";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {
				$sql .= "-IFNULL((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			$sql .= "), 2) SEPARATOR '<br>-') FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id AND op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '<br>-') AS product_cost, 
			GROUP_CONCAT((SELECT GROUP_CONCAT(ROUND(o.currency_value*((op.total";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			$sql .= ")-((opc.cost*op.quantity)";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {
				$sql .= "-IFNULL((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			$sql .= ")), 2) SEPARATOR '<br>') FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id AND op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '<br>') AS product_profit, 
			GROUP_CONCAT((SELECT GROUP_CONCAT(IFNULL(ROUND(100*((op.total";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			$sql .= ")-((opc.cost*op.quantity)";
			...
			$sql .= "), 2),0) SEPARATOR '%<br>') FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id AND op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '%<br>') AS product_profit_margin, 
			GROUP_CONCAT((SELECT GROUP_CONCAT(IFNULL(ROUND(100*((op.total";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id =  '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id),0)";
			}						
			...
			$sql .= "), 2),0) SEPARATOR '%<br>') FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id AND op.order_id = o.order_id ORDER BY op.order_product_id) ORDER BY o.order_id DESC SEPARATOR '%<br>') AS product_profit_markup, 

			GROUP_CONCAT(o.order_id ORDER BY o.order_id DESC SEPARATOR '<br>') AS customer_ord_id, 
			GROUP_CONCAT('<a href=\"index.php?route=sale/order/info&user_token=$user_token&order_id=',o.order_id,'\">',o.order_id,'</a>' ORDER BY o.order_id DESC SEPARATOR '<br>') AS customer_ord_id_link, ";
			if ($this->config->get('advsop' . $this->user->getId() . '_date_format') == 'DDMMYYYY') {
			...
			$sql .= "GROUP_CONCAT(IF (o.customer_id = 0,'0',CONCAT('<a href=\"index.php?route=customer/customer/edit&user_token=$user_token&customer_id=',o.customer_id,'\">',o.customer_id,'</a>')) ORDER BY o.order_id DESC SEPARATOR '<br>') AS customer_cust_id_link, 
		...
		if (isset($data['filter_report']) && $data['filter_report'] == 'coupon') {
			...
			$sql .= "FROM `" . DB_PREFIX . "order` o" . $date . $sdate . $osi . $order_id . $order_value . $store . $cur . $tax . $tclass . $geo_zone . $cgrp . $cust . $email . $tel . $ip . $pcomp . $paddr . $pcity . $pzone . $ppsc . $pcntr . $pmeth . $scomp . $saddr . $scity . $szone . $spsc . $scntr . $smeth . $cat . $manu . $supl . $sku . $prod . $mod . $opt . $atr . $loc . $cpn . $cpc . $gvc . $cgn . $cgc;
		}
		}
		...
		if (in_array('all_prod_option', $this->config->get('advsop' . $this->user->getId() . '_settings_all_columns'))) {		
		...
		if (in_array('all_prod_attributes', $this->config->get('advsop' . $this->user->getId() . '_settings_all_columns'))) {		
		...
		(SELECT s.name FROM `" . DB_PREFIX . "supplier` s, `" . DB_PREFIX . "order_product_cost` opc WHERE opc.supplier_id = s.supplier_id AND op.order_product_id = opc.order_product_id) AS product_supplier, 
		...
		(SELECT ((opc.base_price-op.price)*op.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_product_id = opc.order_product_id) AS product_discount, 
		(SELECT (o.currency_value*((opc.base_price-op.price)*op.quantity)) FROM `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_product_id = opc.order_product_id) AS product_discount_raw, 
		IFNULL((SELECT SUM(r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "'), 0) AS product_qty_refund, 
		(SELECT (op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "') AS product_refund, 
		(SELECT o.currency_value*(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "') AS product_refund_raw, 
		op.reward AS product_reward_points, 
		(op.total";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
			$sql .= "-IFNULL((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "'),0)";
		}						
		...
		$sql .= ") AS product_sales_raw, 
		((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id)";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {
			$sql .= "-IFNULL((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND opc.order_product_id = op.order_product_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "'),0)";
		}						
		$sql .= ") AS product_cost, 
		(o.currency_value*(SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc WHERE opc.order_product_id = op.order_product_id)";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {
			$sql .= "-IFNULL((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND opc.order_product_id = op.order_product_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "'),0)";
		}						
		$sql .= ") AS product_cost_raw, 
		...
		(SELECT SUM((opc.base_price-op.price)*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id GROUP BY op.order_id) AS order_discount, 
		(SELECT o.currency_value*SUM((opc.base_price-op.price)*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id GROUP BY op.order_id) AS order_discount_raw, 
		(SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND o.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id) AS order_refund, 
		(SELECT o.currency_value*SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND o.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id) AS order_refund_raw, 
		(SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id) AS order_product_costs_returns, 
		(SELECT o.currency_value*SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY r.order_id) AS order_product_costs_returns_raw, 	
		(o.total) AS order_value, 
		(o.currency_value*o.total) AS order_value_raw, 
		((IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'sub_total' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'handling' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'low_order_fee' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'reward' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'coupon' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'credit' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'voucher' GROUP BY ot.order_id),0)";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_1')) {
			$sql .= "+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'shipping' GROUP BY ot.order_id),0)";
		}
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
		...
		$sql .= ")) AS order_sales_raw, 
		(SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND op.order_product_id = opc.order_product_id GROUP BY op.order_id) AS order_product_costs, 
		(SELECT o.currency_value*SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND op.order_product_id = opc.order_product_id GROUP BY op.order_id) AS order_product_costs_raw, 		
		(o.commission) AS order_commission, 
		(o.currency_value*o.commission) AS order_commission_raw, 
		(SELECT oc.payment_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_payment_cost, 
		(SELECT (o.currency_value*oc.payment_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_payment_cost_raw, 
		(SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_shipping_cost, 
		(SELECT (o.currency_value*oc.shipping_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_shipping_cost_raw, 
		(SELECT oc.extra_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_extra_cost, 
		(SELECT (o.currency_value*oc.extra_cost) FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id) AS order_extra_cost_raw, 
		(SELECT SUM(rc.cost) FROM `" . DB_PREFIX . "return_cost` rc WHERE rc.order_id = o.order_id GROUP BY rc.order_id) AS order_return_cost, 
		(SELECT o.currency_value*SUM(rc.cost) FROM `" . DB_PREFIX . "return_cost` rc WHERE rc.order_id = o.order_id GROUP BY rc.order_id) AS order_return_cost_raw, 
		((IFNULL((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id),0)+o.commission";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
			$sql .= "+(SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
		}
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {
		...
		$sql .= ")) AS order_costs, 
		(o.currency_value*(IFNULL((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id),0)+o.commission";
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
			$sql .= "+(SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id)";
		}
		if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {
		...
		switch($group) {
			...
			case 'week':
				if (isset($data['filter_report']) && $data['filter_report'] == 'tax') {
					if ($this->config->get('advsop' . $this->user->getId() . '_week_days') == 'mon_sun') {
		...
		if (isset($data['filter_sort']) && $data['filter_sort'] == 'report_type') {
			...
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_6')) {
				$sql .= "-IFNULL(SUM((SELECT SUM(op.price*r.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "return` r WHERE r.product_id = op.product_id AND r.order_id = op.order_id AND o.order_id = op.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY op.order_id)),0)";
			}				
			$sql .= ")" . $sort_order . " ";
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'prod_costs') {
			$sql .= " ORDER BY (IFNULL(SUM((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id GROUP BY op.order_id)),0)";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {		
				$sql .= "-IFNULL(SUM((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY opc.order_id)),0)";
			}			
			$sql .= ")" . $sort_order . " ";	
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'commission') {
			$sql .= " ORDER BY commission" . $sort_order . " ";			
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'pay_costs') {
			$sql .= " ORDER BY payment_cost" . $sort_order . " ";
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'ship_costs') {
			$sql .= " ORDER BY shipping_cost" . $sort_order . " ";
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'extra_cost') {
			$sql .= " ORDER BY extra_cost" . $sort_order . " ";
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'return_cost') {
			$sql .= " ORDER BY return_cost" . $sort_order . " ";		
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'ship_balance') {
			$sql .= " ORDER BY SUM((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'shipping' GROUP BY ot.order_id))-SUM((SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id))" . $sort_order . " ";		
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'total_costs') {
			$sql .= " ORDER BY (IFNULL(SUM((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id GROUP BY op.order_id)),0)+SUM(o.commission)";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
				$sql .= "+SUM((SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id))";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {	
				$sql .= "+SUM((SELECT oc.payment_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id))";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_4')) {		
				$sql .= "+SUM((SELECT oc.extra_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id))";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_5')) {		
				$sql .= "+IFNULL(SUM((SELECT SUM(rc.cost) FROM `" . DB_PREFIX . "return_cost` rc WHERE rc.order_id = o.order_id GROUP BY rc.order_id)),0)";
			}	
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_7')) {		
				$sql .= "-IFNULL(SUM((SELECT SUM(opc.cost*r.quantity) FROM `" . DB_PREFIX . "order_product_cost` opc, `" . DB_PREFIX . "return` r WHERE r.product_id = opc.product_id AND r.order_id = opc.order_id AND o.order_id = opc.order_id AND r.return_action_id = '" . $this->config->get('advsop' . $this->user->getId() . '_return_action_refund') . "' GROUP BY opc.order_id)),0)";
			}
			...
			$sql .= ")";			
			$sql .= "-(IFNULL(SUM((SELECT SUM(opc.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order_product_cost` opc WHERE op.order_id = o.order_id AND opc.order_product_id = op.order_product_id GROUP BY op.order_id)),0)+SUM(o.commission)";
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_2')) {
				$sql .= "+SUM((SELECT oc.shipping_cost FROM `" . DB_PREFIX . "order_cost` oc WHERE oc.order_id = o.order_id))";
			}
			if ($this->config->get('advsop' . $this->user->getId() . '_formula_3')) {	
			...
			$sql .= "))" . $sort_order . " ";
		} elseif (isset($data['filter_sort']) && $data['filter_sort'] == 'total_profit_prc') {
	...
	public function getSalesProfitTotal($data = array()) {
		...
		if (!empty($data['filter_supplier'])) {
			$sql .= " AND (SELECT DISTINCT opc.order_id FROM `" . DB_PREFIX . "order_product_cost` opc WHERE o.order_id = opc.order_id AND (";
			$implode = array();
			foreach ($data['filter_supplier'] as $supplier) {
				$implode[] = "opc.supplier_id = '" . (int)$supplier . "'";
			}
		...
		if ($this->config->get('advsop' . $this->user->getId() . '_settings_filters') and (in_array('campaign_name', $this->config->get('advsop' . $this->user->getId() . '_settings_filters')))) {		
		if (!empty($data['filter_campaign_name'])) {
			$sql .= " AND (SELECT DISTINCT mg.marketing_id FROM `" . DB_PREFIX . "marketing` mg WHERE o.marketing_id = mg.marketing_id AND (";
			$implode = array();
			foreach ($data['filter_campaign_name'] as $campaign_name) {
				$implode[] = "o.marketing_id = '" . (int)$campaign_name . "'";
			}
		...
		if (!empty($data['filter_campaign_code'])) {
        	$sql .= " AND (SELECT DISTINCT mg.marketing_id FROM `" . DB_PREFIX . "marketing` mg WHERE o.marketing_id = mg.marketing_id AND LCASE(mg.code) LIKE '%" . $this->db->escape(mb_strtolower($data['filter_campaign_code'], 'UTF-8')) . "%')";	
		} else {
			$sql .= '';
		}
	...
	if (in_array('all_custom_fields', $this->config->get('advsop' . $this->user->getId() . '_settings_all_columns'))) {	
