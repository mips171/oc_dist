	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false)
	{
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				// if payment_method is credit_checkout deduct the used credit from the customer's credit balance
				if ($order_info['payment_method'] == 'credit_checkout') { // Adjust the comparison as needed
					$customer_balance = $this->db->query("SELECT credit FROM " . DB_PREFIX . "customer WHERE customer_id = " . (int) $order_info['customer_id']);
					if ($customer_balance->RecordCount() > 0) {
						$new_balance = $customer_balance->fields['credit'] - $order_info['order_total'];

						// Optional: Check for negative balance
						if ($new_balance >= 0) {
							$this->db->query("UPDATE " . DB_PREFIX . "customer SET credit = " . $new_balance . " WHERE customer_id = " . (int)$order_info['customer_id']);
						} else {
							// Handle negative balance scenario, e.g., throw an error or alert the user
						}
					}
				}
			}
