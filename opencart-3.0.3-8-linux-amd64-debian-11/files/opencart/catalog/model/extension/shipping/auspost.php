...
class ModelExtensionShippingAusPost extends Model
{
	...
	// 				foreach ($this->cart->getProducts() as $product) {
	// 					$fromUnit = $this->getLengthUnitByID($product['length_class_id']);
	// 					$toUnit = self::CENTIMETER_UNIT; // Since the AusPost API always wants centimeters
	...
	// 					// fallback weight
	// 					$formatted_weight = self::FALLBACK_WEIGHT;
	// 				}
	...
	// 				$full_url = self::AUSPOST_API_BASE . self::AUSPOST_API_SHIPPING_ENDPOINT;

	// 				if ($this->config->get('shipping_auspost_testmode')) {
	// 					$full_url = self::AUSPOST_API_BASE . self::AUSPOST_API_TEST . self::AUSPOST_API_SHIPPING_ENDPOINT;
	// 				}

	// 				// Capture and log the request details
	// 				$logMessage = "Endpoint URL: " . $full_url . "\n";
	// 				$logMessage .= "Payload: " . json_encode($shipment_data) . "\n";
	// 				file_put_contents(self::ERROR_LOG_PATH, $logMessage, FILE_APPEND);
	...
	// 						file_put_contents(self::ERROR_LOG_PATH, $response, FILE_APPEND);
	// 					}
