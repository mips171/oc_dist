...
class ControllerJournal3Gallery extends ModuleController {
	...
	protected function parseGeneralSettings($parser, $index) {
		$data = array(
			...
			'images'          => array(),
			'options'         => array(
				'mainClass' => 'fancybox-' . $this->module_id,
...
//				'allowMediaOverlap' => $parser->getSetting('moduleGalleryThumbToggleStatus'),
		...
		if ($parser->getSetting('type') == 'link') {
			...
			if ($parser->getSetting('type') === 'image') {
				...
				$this->settings['images'][] = array(
					...
					'caption' => $parser->getSetting('title'),
				);
			} else {
	...
	protected function afterRender() {
		...
		$this->document->addScript('catalog/view/theme/journal3/lib/fancybox/fancybox.min.css', 'lib-fancybox');
		$this->document->addScript('catalog/view/theme/journal3/lib/fancybox/fancybox.min.js', 'lib-fancybox');
	}
